...
// reCaptcha secret
var reCaptchaSecret = process.env.GRCSECRET;
...
function OnConnect(socket) {
    try {
        ...
        socket.emit('allow', {
            keyString: (process.env.YTAPIKEY || "NOKEY")
        });
        ...
        setInterval(function() {
            ...
            if (spamPoints < 0) {
                spamPoints = 0;
            } else if (spamPoints > 10 && !room) {
                socket.emit('information', "[INFO] You've been kicked for spamming the chat.");
                socket.disconnect();
            }
        }, 2000);
        ...
        socket.on('login', function(data) {
            ...
            if (typeof reCaptchaSecret === 'undefined') {
                loggedIn = true;

                finishLogin(data);
            } else if (data.reCaptchaResponse == null || typeof data.reCaptchaResponse == 'undefined') {
                console.log("@ " + ip + ": Attempted crash using invalid data.");
                socket.disconnect();
                return;
            } else {
                    ...
                    if (message.lastIndexOf('/svrmsg ', 0) === 0 && (user.admin || user.mod)) {
                        ...
                        if (!userWanted) {
                            ...
                            if (roomfound) {
                                ...
                                userWanted.socket.emit('openroom', {
                                    roomtoken: newroom.token,
                                    usertoken: userWanted.token
                                });
                            ...
                            if (roomfound) {
                                if (roomfound.users.indexOf(user) != -1) {
                                    ...
                                    for (var j = 0; j < usersWanted.length; j++) {
                                        ...
                                        usersWanted[j].socket.emit('openroom', {
                                            roomtoken: roomfound.token,
                                            usertoken: usersWanted[j].token
                                        });
        ...
        if (err) {
            ...
            database.News.find({
                ...
                if (err) {
                    ...
                    for (var i = 0; i < users.length; i++) {
                        users[i].socket.emit('newsupdate', {
                            array: currentNews
                        });
                    }
                }
...
function generateRoster(from) {
    ...
    for (var i = 0; i < from.length; i++) {
        if (from[i].inBigChat) {
            roster.push({
                nick: from[i].nick,
                gender: getGenderSymbol(from[i].gender),
                role: getRoleSymbol(from[i].role),
                authority: getAuthority(from[i]),
                afk: from[i].AFK
            });
        }
    }
